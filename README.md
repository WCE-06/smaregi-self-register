...
- 決済ボタン操作後は登録済みの`テンキー5`座標をクリックし、ポインターを決済ボタン上から安全な待機位置へ退避する
- 商品ジョブは会員操作の完了を待たず、会員ジョブがキューへ登録された直後に次のジョブとして予約する
- 税抜商品は税率別の税抜小計へ税率を掛け、スマレジと同じ端数処理（既定は四捨五入）で税込合計を算出する
- FEBBRAIOは参照専用APIから利用中判定・経過分数・会員区分・STN/STR料金商品コードを取得する
- 受付側がAPI呼び出し時点までの経過時間を60分単位で切り上げ、通常会員はSTN01～10、住民会員はSTR01～10を返す
- セルフレジは返されたコードを商品マスタと照合してスマレジへ1点登録する
- FEBBRAIOの利用先ボタンは常に選択可能とし、押下後に受付APIで利用中記録を確認する。料金商品は`STN01～10／STR01～10`の商品コードで照合する
- 現金投入・キャッシュレス種別選択・電子マネー種別選択・決済完了待ちでは、BLE HID接続維持のため20秒ごとに`テンキー5`を1回送る。通常ジョブ実行中、非表示タブ、待機画面以外では送らない
