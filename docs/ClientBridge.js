...
window.RegisterBridge = (() => {
  ...
  function cancel(businessKey, context) {
    ...
    if (!nativeAvailable()) return remoteCall('cancel', {businessKey:String(businessKey || createBusinessKey()),context:cancelContext});
    return new Promise((resolve, reject) => {
      google.script.run
        .withSuccessHandler(resolve)
        .withFailureHandler(error => reject(normalizeError(error)))
        .enqueueCancelScenario(String(businessKey || createBusinessKey()), GAS_UI_TOKEN, cancelContext);
    });
  }
  ...
  async function watch(jobId, onUpdate, options) {
    ...
    const intervalMs = Number(options && options.intervalMs || 150);
